...
# These questions use a database called SQLite. You do not need to install
# In the file `pets_db.py` three tables are created. Data is then added to each
# of the tables. The questions below are about how the data in each of the
# tables is related.

# Part 4.A:
# Write SQL to select the pets that are owned by nobody.
# The output should be a list of tuples in the format: (<pet name>, <species>,
# <age>)
...
# The output should be a list of tuples in the format: (<person name>,
# <pet name>, <species>)
...
AND (
  SELECT COUNT(*)
  FROM people_animals
  WHERE pet_id = animals.animal_id
) = 1;
